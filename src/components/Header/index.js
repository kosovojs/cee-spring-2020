...
function Header() {
	...
	useEffect(() => {
		async function fetchData() {
		  const {date} = await api.tool.updated();
		  setUpdated(date);
		}
		fetchData();
	}, []);
		...
		<div className={classes.grow}>
			<AppBar position='sticky' className={classes.removedShadow}>
				<Toolbar>
					...
					<MenuItem onClick={() => window.open(`https://github.com/kosovojs/cee-spring-2020`)}>
						<GitHubIcon />
					</MenuItem>
					<MenuItem onClick={() => window.open(`https://github.com/kosovojs/cee-spring-2020/issues`)}>
						<BugReportIcon />
					</MenuItem>
				</Toolbar>
			</AppBar>
